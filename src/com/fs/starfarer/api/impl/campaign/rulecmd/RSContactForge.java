...
public class RSContactForge extends BaseCommandPlugin{
    protected PersonAPI person;
    @Override
    public boolean execute(String ruleId, InteractionDialogAPI dialog, List<Misc.Token> params,
                           Map<String, MemoryAPI> memoryMap) {
        dialog.showCustomProductionPicker(new RSContactForgeDelegate(dialog,memoryMap));
        return false;
    }

    public static class RSContactForgeDelegate extends BaseCustomProductionPickerDelegateImpl {
        ...
        public RSContactForgeDelegate(InteractionDialogAPI dialog,Map<String,MemoryAPI> memoryMap){
...
//            this.high = this.level>=HSIHPSIDLevelIntel.Stage.SENIOR_SHIP_PROVIDER.ordinal();
        ...
        public Set<String> getAvailableShipHulls() {
            ...
            for(ShipHullSpecAPI spec:Global.getSettings().getAllShipHullSpecs()){
                if(spec.getTags().contains("rs_shiyan_bp")){
                    toR.add(spec.getHullId());
                }
                if(high&&spec.getTags().contains("rs_praetorianorum_bp")){
                    toR.add(spec.getHullId());
                }
                if(high&&spec.getTags().contains("subflagship")){
                    toR.add(spec.getHullId());
                }
        ...
        public Set<String> getAvailableWeapons() {
            ...
            for(WeaponSpecAPI spec:Global.getSettings().getAllWeaponSpecs()){
                ...
                if(spec.getTags().contains("rs_bp")&&!spec.getTags().contains("rs_praetorianorum_bp")){
        ...
        public Set<String> getAvailableFighters() {
            ...
            for(FighterWingSpecAPI spec:Global.getSettings().getAllFighterWingSpecs()){
                ...
                if(spec.getTags().contains("rs_bp")&&!spec.getTags().contains("rs_shiyan_bp")){
                    toR.add(spec.getId());
                }
                if(high&&spec.getTags().contains("rs_praetorianorum_bp")){
                    toR.add(spec.getId());
...
//            Misc.adjustRep("HSI",Math.min(0.1f,cost/200000f),null);
            FireBest.fire(null, dialog, memoryMap, "RSForgePicked");
        }
    }
}
