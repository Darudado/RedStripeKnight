...
public class Moci_Blade_BeamEffect_MA implements BeamEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
        ...
        if (beam.getBrightness() >= 0.7f && target != null) {
            ...
            float dmg = weapon.getDamage().getDamage()/300;

            engine.applyDamage(target, beam.getTo(), dmg, this.weapon.getDamageType(), dmg, false, true, this.weapon.getShip());
            // 计算当前距离
        ...
        if (weapon.getSlot().getId().endsWith("_LEFTHAND") || weapon.getSlot().getId().endsWith("_RIGHTHAND")) {
