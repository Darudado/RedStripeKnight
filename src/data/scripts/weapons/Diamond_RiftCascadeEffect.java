...
public class Diamond_RiftCascadeEffect implements BeamEffectPlugin { //WithReset {

	public static Color NEGATIVE_SOURCE_COLOR = new Color(242,110,100,75);
...
//	}
	...
	public Diamond_RiftCascadeEffect() {
...
//		if (!canSpawn || beam.getBrightness() >= 1f) return;
		...
		if (beam.getLength() > beam.getWeapon().getRange() - 10f) {
			...
			if (dist < SPAWN_SPACING * 2f) {
				...
				arc.setCoreWidthOverride(Math.max(15f, thickness));
...
//		Vector2f perp2 = new Vector2f(dir.y, -dir.x);
		...
		for (int i = 0; i < 3; i++) {
			...
			//dur *= 2f;
			float size = 200f + 55f * (float) Math.random();
			size *= sizeMult;
			//size *= 0.5f;
		...
		if (length > spawnOtherParticleRange * 2f && (float) Math.random() < 0.25f) {
			...
			color = new Color(173,14,14,75);
			//color = new Color((int)((float) Math.random()*225f),(int)((float) Math.random()*225f),(int)((float) Math.random()*225f),(int)((float) Math.random()*225f));
			int numToSpawn;
			numToSpawn = 1;
			for (int i = 0; i < numToSpawn; i++) {
				...
				for (int j = 0; j < 7; j++) {
					float rampUp = 0.25f + 0.25f * (float) Math.random();
					float dur = 1f + (float) Math.random();
					float size = 75f + 50f * (float) Math.random();
					Vector2f loc = new Vector2f(dir);
					float sign = Math.signum((float) Math.random() - 0.5f);
					loc.scale(distAlongBeam*0.9f + sign * (float) Math.random() * size * 0.5f);
...
//		float dir = Misc.getClosestTurnDirection(beamAngle, beamSourceToTarget);
		...
		if (anglePerSegment > 360f) anglePerSegment = 0f;
...
//		}
		...
		if (target instanceof ShipAPI ship && !hitShield) {
			...
			if (bounds != null) {
				...
				if (best != null) {
					...
					if (o instanceof MissileSpecAPI spec) {
...
//						Vector2f.add(prevMineLoc, dir, dir);
	...
	public float getSizeMult() {
		...
		sizeMult = 0.6f + (1f - sizeMult) * 0.5f;
		//sizeMult = 0.5f + 0.5f * sizeMult;
		return sizeMult;
	}
	public void spawnMine(ShipAPI source, Vector2f mineLoc) {
		CombatEngineAPI engine = Global.getCombatEngine();
		MissileAPI mine = (MissileAPI) engine.spawnProjectile(source, null, RIFTCASCADE_MINELAYER, mineLoc, (float) Math.random() * 360f, null);
		float sizeMult = getSizeMult();
		if (sizeMult > 10f) sizeMult=10f;
		mine.setCustomData(com.fs.starfarer.api.impl.combat.RiftCascadeMineExplosion.SIZE_MULT_KEY, sizeMult);
		//mine.setCustomData(data.weapons.D_RiftCascadeMineExplosion.SIZE_MULT_KEY, sizeMult);
		if (source != null) {
			Global.getCombatEngine().applyDamageModifiersToSpawnedProjectileWithNullWeapon(source, WeaponType.ENERGY, false, mine.getDamage());
		}
		mine.getDamage().getModifier().modifyMult("mine_sizeMult", sizeMult*1.15f);
