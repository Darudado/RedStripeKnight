 ...
 */
public class Moci_Owsla_LineBit_Advanced implements EveryFrameWeaponEffectPlugin {
    private static final int RANGE_THRESHOLD = 750; // 触发模块分离的距离阈值
    private static final float DEFAULT_RETURN_DELAY = 10f; // 默认模块自由活动时间（秒）
    ...
    public Moci_Owsla_LineBit_Advanced() {
    }
    ...
    private static class moduleController implements AdvanceableListener {
        ...
        private final Moci_Owsla_LineBit_Advanced parentInstance; // 外部类实例引用
        ...
        private moduleController(String id, ShipAPI ship, ShipAPI source, WeaponAPI weapon, ShipAPI target,
                                 float returnDelay, Moci_Owsla_LineBit_Advanced parentInstance) {
            this.id = id;
            this.ship = ship;
            this.source = source;
            this.weapon = weapon;
