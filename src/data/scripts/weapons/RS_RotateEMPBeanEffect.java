 ...
 */
public class RS_RotateEMPBeanEffect implements BeamEffectPlugin {

    // ============= 【旋转点位相关常量】- 可直接修改自定义旋转效果 =============
    private static final float ROTATION_DISTANCE = 20f;      // 旋转半径（像素）：点位离发射点的距离，越大电弧越远
    private static final float ROTATION_SPEED = -5f;        // 旋转速度（度/0.05秒）：负数=顺时针旋转，正数=逆时针，绝对值越大越快
    ...
    private static final float SLOP_RANGE = 75F;            // 电弧超射程冗余距离：防止目标贴脸时无法击中
    ...
    private static final float INITIAL_DAMAGE = 30F;      // 电弧基础伤害值（能量伤害）
    private static final float INITIAL_EMP = 15F;           // 电弧基础EMP值（电磁瘫痪，此处设0）
    ...
    private static final float DAMAGE_ENHANCE_RATE = 2f;   // 链式闪电伤害增强倍率：每弹射一次，伤害提升20%
    ...
    private static final float CHAIN_ARC_THICKNESS = 20f;    // 链式电弧的基础厚度
    ...
    private void fireLightningArc(CombatEngineAPI engine, ShipAPI ship, WeaponAPI weapon,
        ...
        EmpArcEntityAPI arc = engine.spawnEmpArcVisual(spawnLocation, ship, visualPoint, target, thickness,
                new Color(195,170,35, clamp255(brightness)), // 电弧边缘色：淡紫
                new Color(195,140,50, clamp255(brightness)),    // 电弧核心色：深紫
                arcParams);
        if (arc != null) {
        ...
        if (target != null) {
            ...
            for (int i = 0; i < 15; ++i) {
                ...
                Color sparkColor = new Color(MathUtils.getRandomNumberInRange(165, 200),
                        MathUtils.getRandomNumberInRange(140, 175), 50);
            ...
            if (!(target instanceof ShipAPI)) {
                ...
                engine.addHitParticle(point, target.getVelocity(), hitGlowSize, 1.0F, new Color(195,140,50, 90));
            }
    ...
    private void spawnChainArc(CombatEngineAPI engine, ShipAPI source, Vector2f start,
        ...
        EmpArcEntityAPI arc = engine.spawnEmpArc(source, start, source, target,
                ...
                thickness, new Color(195,140,50, 90),
                new Color(195,170,35, 90), params);
        if (arc != null) {
