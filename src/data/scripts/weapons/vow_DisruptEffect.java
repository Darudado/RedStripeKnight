...
import static data.scripts.CombatPlugin.addDisrput;

public class vow_DisruptEffect implements EveryFrameWeaponEffectPlugin {

    public final float degrees = 20;
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        for (Iterator<Map.Entry<ShipAPI, Float>> iter = debuffed_fighters.entrySet().iterator(); iter.hasNext(); ) {
            ...
            if (!entry.getKey().isAlive() || entry.getValue() - amount < 0) {
                iter.remove();
                entry.getKey().getMutableStats().getTimeMult().unmodify("vow_Disrupt" + weapon.hashCode());
            } else {
                entry.setValue(entry.getValue() - amount);
                entry.getKey().setJitterUnder(entry.getKey(), new Color(44, 255, 255), 4, 8, 2);
                entry.getKey().getMutableStats().getTimeMult().modifyMult("vow_Disrupt" + weapon.hashCode(), 0.80f);
            }
        }
