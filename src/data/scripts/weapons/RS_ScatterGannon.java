...
public class RS_ScatterGannon implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
    ...
    public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine)
    {
        ...
        int shotCount1 = (9); //Scatter projectiles
        for (int j = 0; j < shotCount1; j++) {
            ...
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "rs_scattercannon_sub", loc, proj.getFacing(),
                    randomVel);
        }
        int shotCount2 = (1); //Core projectile
        for (int j = 0; j < shotCount2; j++) {
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "rs_scattercannon_core", loc, proj.getFacing(),
                    null);
        }
        int shotCount3 = (12); //Micro projectiles
        for (int j = 0; j < shotCount3; j++) {
            ...
            randomVel.y += vel.y;
            //spec + "_clone" means this will call the weapon (not projectile! you need a separate weapon) with the id "($projectilename)_clone".
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "rs_scattercannon_submicro", loc, proj.getFacing(),
                    randomVel);
        }
