...
public class IgnisBeamEffect implements BeamEffectPlugin {
    private final IntervalUtil fireInterval = new IntervalUtil(0.15F, 0.3F);
    IntervalUtil sparkInterval = new IntervalUtil(0.15F, 0.3F);
    private boolean wasZero = true;
    public static final float maxSpread = 10;

    public IgnisBeamEffect() {
    }
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
        ...
        if (target instanceof ShipAPI && beam.getBrightness() >= 1.0F) {
            ...
            if (this.fireInterval.intervalElapsed()) {
                ...
                float emp = beam.getDamage().getFluxComponent() * 0.75F;
                float dam = beam.getDamage().getDamage() * 0.05F;
                ...
                engine.spawnEmpArcPierceShields(beam.getSource(), point, beam.getDamageTarget(), beam.getDamageTarget(), DamageType.ENERGY, dam/2, emp/2, 100000.0F, "tachyon_lance_emp_impact", (beam.getWidth() + 9.0F)/2, beam.getFringeColor(), beam.getCoreColor());
                engine.spawnEmpArcPierceShields(beam.getSource(), point, beam.getDamageTarget(), beam.getDamageTarget(), DamageType.ENERGY, dam/4, emp/4, 100000.0F, "tachyon_lance_emp_impact", (beam.getWidth() + 9.0F)/4, beam.getFringeColor(), beam.getCoreColor());
                engine.spawnEmpArcPierceShields(beam.getSource(), point, beam.getDamageTarget(), beam.getDamageTarget(), DamageType.ENERGY, dam/8, emp/8, 100000.0F, "tachyon_lance_emp_impact", (beam.getWidth() + 9.0F)/8, beam.getFringeColor(), beam.getCoreColor());
            }
        }
        ...
        if (beam.didDamageThisFrame()){
            ...
            float dam = beam.getDamage().getDamage() * 0.05F;
            Vector2f point = beam.getRayEndPrevFrame();
            engine.spawnEmpArcPierceShields(beam.getSource(), point, beam.getDamageTarget(), beam.getDamageTarget(), DamageType.KINETIC, dam/8, emp/8, 100000.0F, "tachyon_lance_emp_impact", (beam.getWidth() + 9.0F)/8, beam.getFringeColor(), beam.getCoreColor());
        }
