...
public class MobileArmmorGroundSupport extends AbilityPlugin {

    public static final float ENTRY_HEIGHT = 60;
    public static final float BUTTON_WIDTH = 80;
    public static float BASE_DAMAGE = 75;    // for comparison, fire support on a size 6 does 72
    public static int NUM_HITS = 1;
    public static float CR_TO_FIRE = 0.4f;
    public static float DISRUPT_TIME_MULT = 0.25f;

    public static final Set<String> CARRIER_IDS = new HashSet<>();

    static {
        CARRIER_IDS.add("rs_typhaon");
        CARRIER_IDS.add("rs_sophia");
        CARRIER_IDS.add("rs_bethlehem");
        CARRIER_IDS.add("rs_lordanes");
        CARRIER_IDS.add("vow_piafides");
        CARRIER_IDS.add("vow_tentatio");
        CARRIER_IDS.add("vow_admonitio");
    }

    public transient FleetMemberAPI MobileWeapon;

    public float getDamage(FleetMemberAPI MobileWeapon) {
        float damage = BASE_DAMAGE + ((MobileWeapon.getMemberStrength()) +
                MobileWeapon.getStats().getDynamic().getMod(Stats.FLEET_GROUND_SUPPORT).getFlatBonus()) * 1.25f;
        if (!MobileWeapon.getCaptain().isDefault())
            damage += MobileWeapon.getCaptain().getStats().getLevel() * 1.75f;
        if (MobileWeapon.getStats().getEnergyWeaponDamageMult().getPercentMods().get("cr_effect") != null)
            damage *= 1 + (MobileWeapon.getStats().getEnergyWeaponDamageMult().getPercentMods().get("cr_effect").getValue()) / 750f;
    ...
    public void activate(InteractionDialogAPI dialog, PersonAPI user) {
        ...
        float damage = getDamage(MobileWeapon);
        ...
        damageTaken = Math.min(0.75f, (1f - attrition) * (enemyStrength / (sideStrength + enemyStrength))) * (1f -
                (MobileWeapon.getCaptain().getStats().getLevel() / 100f) * 1.5f);
        damageTaken = MathUtils.getRandomNumberInRange(damageTaken / 3, damageTaken);
        ...
        if (dialog != null) {
            ...
            dialog.getTextPanel().addPara("Inflicted %s damage across %s units. %s suffered %s damage.",
                    h, Math.round(damage) + "", enemies.size() + "", MobileWeapon.getShipName(), Math.round(damageTaken * 100f) + "%");
        ...
        MobileWeapon.getRepairTracker().applyCREvent(-CR_TO_FIRE, "cr_MobiileWeaponStrike", getDef().name);
        MobileWeapon.getStatus().applyHullFractionDamage(damageTaken);
        getIntel().reapply();
    ...
    public void dialogAddVisualPanel(final InteractionDialogAPI dialog) {
        ...
        List<FleetMemberAPI> MobiileWeapon = getMobiileWeapon(Global.getSector().getPlayerFleet());

        List<RadioButtonEntry> allButtons = new ArrayList<>();

        for (final FleetMemberAPI member : MobiileWeapon) {
            ...
            if (enough) {
                ...
                button.setChecked(MobileWeapon == member);
    ...
    protected void dialogSetEnabled(InteractionDialogAPI dialog) {
        dialog.getOptionPanel().setEnabled(AbilityDialogPlugin.OptionId.ACTIVATE, MobileWeapon != null);
    }
    ...
    public void generateTooltip(TooltipMakerAPI tooltip) {
        ...
        tooltip.addPara(str, 0, h, 1 + "", StringHelper.toPercent(attrition),
                StringHelper.toPercent(FireSupportAbilityPlugin.CLOSE_SUPPORT_DAMAGE_MULT - 1));
        tooltip.addPara("MobiileWeapon are inserted into the heart of the combat zone via an assault pod. " +
        ...
        tooltip.addPara("The selected Cataphract will consume %s CR to conduct this attack. In addition, only MobiileWeapon with at least %s HP can be selected.", opad,
    ...
    public Pair<String, Map<String, Object>> getDisabledReason(PersonAPI user) {
        ...
        if (fleet != null) {
            List<FleetMemberAPI> MobiileWeapon = getMobiileWeapon(fleet);
            if (MobiileWeapon.isEmpty()) {
                Map<String, Object> params = new HashMap<>();
    ...
    public boolean showIfDisabled(Pair<String, Map<String, Object>> disableReason) {
        return hasAnyMobiileWeapon(Global.getSector().getPlayerFleet());
    }
    ...
    public boolean aiExecute(GroundBattleAI ai, PersonAPI user) {
        ...
        for (CampaignFleetAPI candidate : fleets) {
            ...
            for (FleetMemberAPI member : getMobiileWeapon(candidate)) {
            ...
            if (MobileWeapon != null) break;
        }

        if (MobileWeapon == null) return false;
        if (fleet != null) {
    ...
    public boolean hasAnyMobiileWeapon(CampaignFleetAPI fleet) {
        for (FleetMemberAPI member : fleet.getFleetData().getCombatReadyMembersListCopy()) {
            if (isMobiileWeapon(member)) return true;
        }
        return false;
    }

    public List<FleetMemberAPI> getMobiileWeapon(CampaignFleetAPI fleet) {
    ...
    public boolean isMobiileWeapon(FleetMemberAPI member) {
        return !(!member.getVariant().getTags().contains("crusaders_ma") && !member.getVariant().hasHullMod("Moci_SpecializedMobileArmour") && !member.getVariant().getTags().contains("moci_ms"));
    }
