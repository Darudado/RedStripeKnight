...
public class CrusadersRaid extends BaseCampaignEventListenerAndScript {
    ...
    private final int maxWait = 12; // Maximum cooldown in months
    private int minWait = 6;  // Minimum cooldown in months
    private final int minHQs = 1;   // Minimum HQ count
    private final int maxHQs = 4;   // HQ count at which cooldown is minimu
    ...
    private static reprisalData getReprisalData() {
        if (Global.getSector().getMemoryWithoutUpdate().get("$armaa_reprisalData") instanceof reprisalData data) {
            return data;
        }
        return null;
    }
    ...
    public int checkHQDisruptionMod() {
        ...
        for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
            if (market.hasIndustry("CR_Defense") && market.getIndustry("CR_Defense").isDisrupted()) {
                extraDays += (int) market.getIndustry("CR_Defense").getDisruptedDays();
            }
        }
    ...
    public int getNumHQs() {
        ...
        int disruptedHQs = 0;
        for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
            if (market.hasIndustry("CR_Defense")) {
                numHQs++;
                if (market.getIndustry("CR_Defense").isDisrupted() || !market.getIndustry("CR_Defense").isFunctional()) {
                    disruptedHQs++;
                }
            }
        }
...
//        }
        ...
        if (data.reprisalTarget == null && data.daysLeft <= 0) {
            ...
            if (Global.getSettings().getModManager().isModEnabled("nexerelin")) {
                ...
                for (FactionAPI fac : Global.getSector().getAllFactions()) {
                    if (fac.getId().equals("cinis_of_crusaders")) {
                        continue;
                    }
                    if (!fac.isHostileTo(Global.getSector().getFaction("cinis_of_crusaders"))) {
                        continue;
                    }
                    if (fac.getMemoryWithoutUpdate().get("$nex_badboy") != null) {
                        badBoys.add(fac);
                        Global.getSector().getFaction("cinis_of_crusaders").setRelationship(fac.getId(), Global.getSector().getFaction("cinis_of_crusaders").getRelationship(fac.getId()) - (float) fac.getMemoryWithoutUpdate().get("$nex_badboy") / 1000f);
                    }
                }
            }
            float sinLevel = -999999f;
            for (MarketAPI market : potMarkets) {
                if (!Global.getSector().getFaction("cinis_of_crusaders").isHostileTo(market.getFaction())) {
                    continue;
                }
                ...
                if (!hasNex || badBoys.isEmpty()) {
                    entity = CrusadersRaidIntel.getEntity(market.getId());
                    break;
                } else {
                    ...
                    if (entity != null && entity.getFaction().getRelationship("cinis_of_crusaders") > -0.50f) {
                        entity.getFaction().setRelationship("cinis_of_crusaders", -0.50f);
                    }
                }
            }
            if (entity == null) {
            ...
            if (Global.getSector().getEconomy().getMarket("exsedol_station_market") == null) {
                for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
                    if (market.hasIndustry("CR_Defense") && !market.getIndustry("CR_Defense").isDisrupted()) {
                        CrusadersRaidIntel.startExpedition(market, "cinis_of_crusaders", entity.getMarket(), new Random());
                        return;
                    }
                }
            } else {
                CrusadersRaidIntel.startExpedition(Global.getSector().getEconomy().getMarket("exsedol_station_market"), "cinis_of_crusaders", entity.getMarket(), new Random());
            }
    ...
    public void reportPlayerEngagement(EngagementResultAPI result) {
        ...
        if(Global.getSector().getPlayerFleet().getFaction().getRelationship("cinis_of_crusaders") <= -0.50)
            return;
        BattleAPI battle = result.getBattle();
        EngagementResultForFleetAPI mrcSide = null;
        EngagementResultForFleetAPI enemySide = null;
        if (result.getLoserResult().getFleet().getFaction().getRelationship("cinis_of_crusaders") <= -0.50) {
        ...
        if (playerDamage > 0 && !data.playerParticipated) {
            data.playerParticipated = true;
        }
        data.playerDamage += playerDamage;
        data.aiDamage += totalDamage - playerDamage;
        System.out.println("Total Damage: " + totalDamage + "Player Damage: " + playerDamage + "Contribution: " + playerDamage / totalDamage);
    }
    ...
    public void reportBattleOccurred(CampaignFleetAPI primaryWinner, BattleAPI battle) {
        ...
        if (primaryWinner.getFaction().equals(Global.getSector().getFaction("cinis_of_crusaders"))
                || primaryWinner.getFaction().getRelationship("cinis_of_crusaders") > 0.10f
                || primaryWinner.getFaction().equals(Global.getSector().getFaction("cinis_of_crusaders"))
