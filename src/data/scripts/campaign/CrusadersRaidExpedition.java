...
public class CrusadersRaidExpedition extends BlockadeFGI implements EconomyTickListener {
    ...
    public static CrusadersRaidExpedition get() {
        return (CrusadersRaidExpedition) Global.getSector().getMemoryWithoutUpdate().get(KEY);
    }


    public CrusadersRaidExpedition(GenericRaidParams params, FGBlockadeParams blockadeParams) {
    ...
    protected CampaignFleetAPI createFleet(int size, float damage) {
        ...
        if (fleet != null)
        {
            ...
            {
                numCrew+= (int) MathUtils.getRandomNumberInRange(member.getMinCrew(),member.getMaxCrew());
            }
            if (size >= 8) {
    ...
    protected void periodicUpdate() {
        ...
        for (CampaignFleetAPI curr : getFleets())
        {
            List<FleetAssignmentDataAPI> assignments = new ArrayList<>();
            ...
            if (curr.getMemoryWithoutUpdate().getBoolean(ARMADA)) {
                ...
                if(curr.getMemoryWithoutUpdate().getBoolean(ARMADA+"_fallBackFleet"))
                {
                    ...
                    for (FleetAssignmentDataAPI assignment : assignments) {
                        curr.addAssignment(assignment.getAssignment(), assignment.getTarget(), assignment.getMaxDurationInDays() - assignment.getElapsedDays(), assignment.getOnCompletion());
                    }
        ...
        if (armada <= 0 || numfleets <= 1)
        {
            ...
            final CampaignFleetAPI meshaniiGuardFleet = MagicCampaign.createFleetBuilder()
                    ...
                    .setMinFP(450)
                    .setReinforcementFaction("red_stripe_vower")
                    .setSpawnLocation(params.source.getPrimaryEntity())
                    .setSupportAutofit(true)
                    .setIsImportant(false)
                    .create();
            meshaniiGuardFleet.setFaction("red_stripe_vower");
        ...
        if (action instanceof FGBlockadePlanetAction blockade) {
...
//		Color s = raidAction.getSystemNameHighlightColor();
    ...
    public void performTakeover(boolean voluntary) {
        ...
        MarketAPI target = blockadeParams.specificMarket;
        target.setFactionId("red_stripe");
