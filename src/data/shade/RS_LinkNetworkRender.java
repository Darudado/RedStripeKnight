...
public class RS_LinkNetworkRender extends BaseCombatLayeredRenderingPlugin {
    ...
    private java.util.List<RenderData> datas = new ArrayList<>();
    public static final String _KEY = "RENDER_KEY";

    public RS_LinkNetworkRender() {
        sprite = Global.getSettings().getSprite("graphics/fx/rs_SKR_beamcore.png");//填入光束材质
        layer = CombatEngineLayers.ABOVE_SHIPS_LAYER;
    }

    public static RS_LinkNetworkRender getInstance(){
        if(Global.getCombatEngine().getCustomData()!=null&&Global.getCombatEngine().getCustomData().containsKey(_KEY)){
            return (RS_LinkNetworkRender) Global.getCombatEngine().getCustomData().get(_KEY);
        }else{
            RS_LinkNetworkRender r = new RS_LinkNetworkRender();
