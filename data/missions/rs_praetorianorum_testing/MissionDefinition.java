package data.missions.rs_praetorianorum_testing;
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		for (String id : Global.getSettings().getAllVariantIds()) {
			...
			if (variant.getHullSpec().getHullId().startsWith(prefix) && variant.getHullSpec().getHullSize() != ShipAPI.HullSize.FIGHTER && !id.startsWith("mission_") && !variant.getHullSpec().hasTag("module")) {
				if (variant.getHullSpec().hasTag("rs_praetorianorum_bp")  || variant.getHullSpec().hasTag("flagship") || variant.getHullSpec().hasTag("subflagship")) {
					variants.add(id);
				}
			}
		}
